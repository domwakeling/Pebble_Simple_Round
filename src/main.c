...
void main_window_load(Window *w) {
	...
	// load font
	arial_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_ARIAL_12));
	...
	for(int i = 0; i < 12; i++) {
		...
		text_layer_set_font(text_layers[i], arial_font);
	...
	// once they're all created, set the text
...
void main_window_unload(Window *w) {
	// destroy the font
	fonts_unload_custom_font(arial_font);
